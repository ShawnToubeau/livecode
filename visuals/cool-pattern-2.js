    ...
    solid()
        .layer(solid().add(
            osc((time/16) * 1, (time/1000) * 0.2  )
                ...
                .diff(shape(106,1,0.05))
        ))
        .modulateScale(osc(2,0.125),0.125)
//
solid()
    .layer(solid(1,1,1).mult(pat().diff(src(o0).scale(0.2).mult(solid(),[0.7,0.6,0.4,0.6]).kaleid(1.01).saturate(0.3)))
        .layer(solid(1,1,1)
            .mask(
                noise([1, 4].smooth().fast(.025),0.05)
